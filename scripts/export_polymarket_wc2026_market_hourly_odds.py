#!/usr/bin/env python3
"""Export WC2026 market hourly odds from the local DuckDB warehouse."""
...
MART_NAME: Final = "polymarket_wc2026_market_hourly_odds"


def export_polymarket_wc2026_market_hourly_odds(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.snapshot_copy:
        ...
        snap_dir = Path(
            tempfile.mkdtemp(
                prefix="polymarket_wc2026_market_hourly_snap_", dir=str(args.output_dir)
            )
        )
        try:
    ...
    try:
        row_count = export_polymarket_wc2026_market_hourly_odds(conn, output_path)
