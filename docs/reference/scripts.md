...
- `export_polymarket_wc2026_match_minute_odds.py`: write the 104-game match-minute mart to a temporary Parquet, validate its grain, 104/248/496 inventory, proposition mix, timing, and immutable results provenance, then atomically replace the prior artifact. It prints completeness, boundary nulls, pair warnings, revision/hash, file size, and SHA-256; quality warnings do not fail export.
