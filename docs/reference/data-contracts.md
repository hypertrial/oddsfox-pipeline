...
| `polymarket_wc2026_market_minute_odds` | One row per `(market_id, odds_minute_epoch)` | Minute odds (unified, isolated) | Minute-grain counterpart of the hourly golden mart. Match-level markets use in-game windows; futures markets use the tournament span `[2026-06-11, 2026-07-19]` capped by each market's close/resolution time. |
