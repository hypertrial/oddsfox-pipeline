...
use crate::normalize::{new_snapshot_id, IngestMetaBuilders, SnapshotRecord};
use crate::schema;
...
pub fn events_from_markets(markets: &[KalshiMarket]) -> Vec<KalshiEvent> {
    ...
    for market in markets {
        ...
        events
            .entry(event_ticker.clone())
            .or_insert_with(|| KalshiEvent {
...
pub fn markets_batch(
    ...
    for market in markets {
        ...
        active.append_value(matches!(
            market.status.as_deref(),
            Some("open" | "paused" | "unopened")
        ));
        closed.append_value(matches!(
            market.status.as_deref(),
            Some("closed" | "settled")
        ));
        resolved.append_value(
            matches!(market.status.as_deref(), Some("settled")) || market.result.is_some(),
        );
...
pub fn outcomes_batch(
    ...
    for market in markets {
        for (idx, side) in ["yes", "no"].iter().enumerate() {
            ...
            is_winner.append_value(
                market
                    .result
                    .as_deref()
                    .is_some_and(|r| r.eq_ignore_ascii_case(side)),
            );
            meta.append(SOURCE, Some(raw_url), Some(raw_sha256), run_id);
        }
    }
...
pub fn resolutions_batch(
    ...
    for market in markets
        .iter()
        .filter(|m| m.result.is_some() || settlement_time(m).is_some())
    {
        market_id.append_value(kalshi_market_id(&market.ticker));
...
pub fn price_points_from_candlesticks(
    candles: &[KalshiCandlestick],
) -> (Vec<PriceHistoryPoint>, Vec<PriceHistoryPoint>, usize) {
    let mut yes = Vec::new();
    let mut no = Vec::new();
    let mut skipped = 0;
...
pub fn trades_batch(trades: &[KalshiTrade], run_id: &str) -> Result<RecordBatch> {
    ...
    for trade in trades {
        let ticker = trade
            .ticker
            .as_deref()
            .or(trade.market_ticker.as_deref())
            .unwrap_or("unknown");
        let side_value = trade
            .taker_side
            .as_deref()
            .unwrap_or("yes")
            .to_ascii_lowercase();
        ...
        let millis = trade
            .created_ts
            .map(|ts| ts * 1000)
            .or_else(|| parse_time(trade.created_time.as_deref()).map(|dt| dt.timestamp_millis()));
        ts.append_value(millis.unwrap_or_else(|| Utc::now().timestamp_millis()));
        append_f64(
            &mut price,
            normalize_price(trade.yes_price_dollars.or(trade.yes_price)),
        );
...
pub fn snapshot_records_from_orderbook(
    ticker: &str,
    envelope: &KalshiOrderbookEnvelope,
) -> Vec<SnapshotRecord> {
...
fn midpoint(
    bid: Option<&super::models::KalshiCandlePrice>,
    ask: Option<&super::models::KalshiCandlePrice>,
) -> Option<f64> {
