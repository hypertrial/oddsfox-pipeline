...
.PHONY: pre-match-elo-acquire pre-match-elo-inspect pre-match-elo-identity-prepare pre-match-elo-identity-review pre-match-elo-identity-compile pre-match-elo-identity-audit pre-match-elo-release
...
PRE_MATCH_ELO_IDENTITY_WORKSPACE ?= artifacts/pre-match-elo/operator/identity-review
PRE_MATCH_ELO_IDENTITY_MAP ?= artifacts/pre-match-elo/operator/team-identity.parquet
PRE_MATCH_ELO_IDENTITY_REVIEW_REPORT ?= artifacts/pre-match-elo/operator/identity-review-report.json
PRE_MATCH_ELO_IDENTITY_AUDIT_ROOT ?= artifacts/pre-match-elo/identity-audit
PRE_MATCH_ELO_IDENTITY_REVIEWER ?= codex-agent
...
pre-match-elo-identity-prepare: runtime-dirs
	@test -n "$(PRE_MATCH_ELO_TARGET_PARQUET)" || (echo "PRE_MATCH_ELO_TARGET_PARQUET is required" >&2; exit 2)
	$(RUN_IN_REPO) "$(PYTHON)" scripts/build_soccer_pre_match_elo_release.py prepare-identities --target-parquet "$(PRE_MATCH_ELO_TARGET_PARQUET)" --source-catalog "$(PRE_MATCH_ELO_SOURCE_CATALOG)" --raw-root "$(PRE_MATCH_ELO_RAW_ROOT)" --output-directory "$(PRE_MATCH_ELO_IDENTITY_WORKSPACE)"

pre-match-elo-identity-review: runtime-dirs
	$(RUN_IN_REPO) "$(PYTHON)" scripts/build_soccer_pre_match_elo_release.py review-identities --workspace "$(PRE_MATCH_ELO_IDENTITY_WORKSPACE)" --reviewer "$(PRE_MATCH_ELO_IDENTITY_REVIEWER)"

pre-match-elo-identity-compile: runtime-dirs
	$(RUN_IN_REPO) "$(PYTHON)" scripts/build_soccer_pre_match_elo_release.py compile-identities --workspace "$(PRE_MATCH_ELO_IDENTITY_WORKSPACE)" --identity-map "$(PRE_MATCH_ELO_IDENTITY_MAP)" --review-report "$(PRE_MATCH_ELO_IDENTITY_REVIEW_REPORT)"

pre-match-elo-identity-audit: runtime-dirs
	@test -n "$(PRE_MATCH_ELO_TARGET_PARQUET)" || (echo "PRE_MATCH_ELO_TARGET_PARQUET is required" >&2; exit 2)
	@test -f "$(PRE_MATCH_ELO_IDENTITY_MAP)" || (echo "reviewed identity map not found: $(PRE_MATCH_ELO_IDENTITY_MAP)" >&2; exit 2)
	$(RUN_IN_REPO) "$(PYTHON)" scripts/build_soccer_pre_match_elo_release.py audit-identities --target-parquet "$(PRE_MATCH_ELO_TARGET_PARQUET)" --source-catalog "$(PRE_MATCH_ELO_SOURCE_CATALOG)" --raw-root "$(PRE_MATCH_ELO_RAW_ROOT)" --identity-map "$(PRE_MATCH_ELO_IDENTITY_MAP)" --identity-review-report "$(PRE_MATCH_ELO_IDENTITY_REVIEW_REPORT)" --output-directory "$(PRE_MATCH_ELO_IDENTITY_AUDIT_ROOT)" $(if $(PRE_MATCH_ELO_BENCHMARK_PATH),--benchmark-path "$(PRE_MATCH_ELO_BENCHMARK_PATH)",)

pre-match-elo-release: runtime-dirs
	@test -n "$(PRE_MATCH_ELO_TARGET_PARQUET)" || (echo "PRE_MATCH_ELO_TARGET_PARQUET is required" >&2; exit 2)
	@test -f "$(PRE_MATCH_ELO_IDENTITY_MAP)" || (echo "reviewed identity map not found: $(PRE_MATCH_ELO_IDENTITY_MAP)" >&2; exit 2)
	@test -f "$(PRE_MATCH_ELO_IDENTITY_REVIEW_REPORT)" || (echo "identity review report not found: $(PRE_MATCH_ELO_IDENTITY_REVIEW_REPORT)" >&2; exit 2)
	$(RUN_IN_REPO) "$(PYTHON)" scripts/build_soccer_pre_match_elo_release.py build --target-parquet "$(PRE_MATCH_ELO_TARGET_PARQUET)" --source-catalog "$(PRE_MATCH_ELO_SOURCE_CATALOG)" --raw-root "$(PRE_MATCH_ELO_RAW_ROOT)" --identity-map "$(PRE_MATCH_ELO_IDENTITY_MAP)" --identity-review-report "$(PRE_MATCH_ELO_IDENTITY_REVIEW_REPORT)" --dataset-version "$(PRE_MATCH_ELO_DATASET_VERSION)" --output-root "$(PRE_MATCH_ELO_OUTPUT_ROOT)" $(if $(PRE_MATCH_ELO_BENCHMARK_PATH),--benchmark-path "$(PRE_MATCH_ELO_BENCHMARK_PATH)",)
