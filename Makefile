...
POLYMARKET_CATALOG_DUCKDB_PATH := $(abspath $(DUCKDB_NAME))
...
GRAPH_REVISION ?=
RELEASE_VERSION ?=
POLYMARKET_CATALOG_RELEASE_ROOT ?= artifacts/polymarket_catalog/releases
...
.PHONY: reference-bundle-load

reference-bundle-load: runtime-dirs
	@test -n "$(REFERENCE_BUNDLE_DIR)" || (echo "REFERENCE_BUNDLE_DIR is required" >&2; exit 2)
	$(RUN_IN_REPO) "$(PYTHON)" scripts/load_reference_bundle.py --bundle "$(REFERENCE_BUNDLE_DIR)" --warehouse "$(DUCKDB_NAME)"
