...
def test_gamma_events_keyset_shared_pagination_params(monkeypatch, tmp_path):
    ...
    assert wc_calls[0].get("volume_min") is None
...
def test_events_params_include_series_id_when_requested():
    from oddsfox_pipeline.ingestion.polymarket import gamma_events

    params = gamma_events._events_params(
        limit=10,
        keyset_closed=None,
    ...
    assert params["series_id"] == "series-1"
