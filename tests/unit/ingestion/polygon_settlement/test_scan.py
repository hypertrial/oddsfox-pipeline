...
import oddsfox_pipeline.ingestion.polymarket.polygon_settlement_scan as polygon_settlement_scan_module
...
def test_receipt_batch_adaptation_fails_closed_and_shrinks_after_retry(
    ...
    monkeypatch.setattr(
        polygon_settlement_scan_module, "monotonic", lambda: next(timestamps)
    )
...
def test_complete_leaf_adapts_or_retains_log_chunk_size(
    ...
    monkeypatch.setattr(
        polygon_settlement_scan_module, "monotonic", lambda: next(clock)
    )
    result = polygon_settlement_module._collect_and_normalize_leaf(
        rpc=rpc,
        manifest=manifest,
...
def test_concurrent_leaf_requeues_range_after_empty_worker_result(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        polygon_settlement_scan_module,
