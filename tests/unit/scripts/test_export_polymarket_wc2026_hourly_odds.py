"""Tests for scripts/export_polymarket_wc2026_hourly_odds.py."""
...
def _load_export_module():
    ...
    from export_polymarket_wc2026_hourly_odds import (
        export_polymarket_wc2026_hourly_odds_parquet,
        mart_exists,
    )

    return export_polymarket_wc2026_hourly_odds_parquet, mart_exists
...
def _create_hourly_mart(
    ...
    mart_name: str = "polymarket_wc2026_token_hourly_odds",
) -> None:
    conn.execute("create schema if not exists polymarket_wc2026_marts")
    conn.execute(
        f"""
        create table polymarket_wc2026_marts.{mart_name} (
            market_id varchar,
...
def test_export_polymarket_wc2026_hourly_odds_parquet_round_trip(
    tmp_path: Path,
) -> None:
    export_polymarket_wc2026_hourly_odds_parquet, mart_exists = _load_export_module()

    db_path = tmp_path / "test.duckdb"
    out_path = tmp_path / "polymarket_wc2026_token_hourly_odds.parquet"
    ...
    try:
        ...
        conn.executemany(
            """
            insert into polymarket_wc2026_marts.polymarket_wc2026_token_hourly_odds values
            (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [
                (
        ...
        row_count, spec_path = export_polymarket_wc2026_hourly_odds_parquet(
            conn, out_path
        )
    finally:
        conn.close()
    ...
    assert "polymarket_wc2026_token_hourly_odds" in spec_text
...
def test_export_hourly_odds_skips_spec_when_disabled(tmp_path: Path) -> None:
    export_polymarket_wc2026_hourly_odds_parquet, _ = _load_export_module()
    ...
    try:
        ...
        _, spec_path = export_polymarket_wc2026_hourly_odds_parquet(
            conn, out_path, write_spec=False
        )
    finally:
        conn.close()
...
def test_export_missing_hourly_mart_raises(tmp_path: Path) -> None:
    ...
    try:
        try:
            export_polymarket_wc2026_hourly_odds_parquet(conn, out_path)
        except LookupError as exc:
            assert "polymarket_wc2026_token_hourly_odds" in str(exc)
        else:
