"""Tests for scripts/repair_wc2026_polymarket_token_sync_ledger.py."""
...
def _load_repair_module(monkeypatch):
    ...
    import repair_wc2026_polymarket_token_sync_ledger as repair
...
def _create_ledger(conn: duckdb.DuckDBPyConnection) -> None:
    conn.execute("create schema wc2026_polymarket_ops")
    conn.execute(
        """
        create table wc2026_polymarket_ops.token_sync_ledger (
    ...
    conn.execute(
        """
        insert into wc2026_polymarket_ops.token_sync_ledger values
...
def test_repair_token_sync_ledger_rebuilds_transactionally(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.execute(
            ...
            "wc2026_polymarket_ops.token_sync_ledger(fully_checked)"
        )
        ...
        rows = conn.execute(
            ...
            from wc2026_polymarket_ops.token_sync_ledger
        ...
        constraints = conn.execute(
            ...
            where schema_name = 'wc2026_polymarket_ops'
...
def test_repair_token_sync_ledger_rejects_bad_schema_before_mutation(
    ...
    try:
        conn.execute("create schema wc2026_polymarket_ops")
        conn.execute(
            "create table wc2026_polymarket_ops.token_sync_ledger (bad varchar)"
        )

        with pytest.raises(RuntimeError, match="Unexpected wc2026_polymarket_ops"):
        ...
        columns = conn.execute(
            ...
            where table_schema = 'wc2026_polymarket_ops'
              and table_name = 'token_sync_ledger'
            """
        ).fetchall()
    finally:
