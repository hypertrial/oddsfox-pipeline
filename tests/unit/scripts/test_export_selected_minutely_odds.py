"""Tests for scripts/export_selected_minutely_odds.py."""
...
def _load_export_module():
    ...
    from export_selected_minutely_odds import (
...
def test_export_minutely_odds_parquet_round_trip(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "selected_token_minutely_odds.parquet"
    ...
    try:
        ...
        conn.execute(
            """
            create table polymarket_marts.selected_token_minutely_odds (
                market_id varchar,
        ...
        conn.executemany(
            """
            insert into polymarket_marts.selected_token_minutely_odds values
            (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [
                (
    ...
    assert "selected_token_minutely_odds" in spec_text
...
def test_export_missing_mart_raises(tmp_path: Path) -> None:
    ...
    try:
        try:
            ...
            assert "selected_token_minutely_odds" in str(exc)
        else:
