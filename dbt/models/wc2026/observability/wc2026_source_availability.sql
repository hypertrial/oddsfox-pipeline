...
private_payload_rows as (
    select
        'eloratings' as source,
...
private_rows as (
    select
        ...
        latest.latest_snapshot_id is not null
        and coalesce(payload.row_count, 0) > 0 as available,
        ...
        coalesce(payload.row_count, 0) as row_count,
    ...
    left join private_payload_rows as payload
        on
            latest.source = payload.source
            and latest.latest_snapshot_id = payload.snapshot_id
),
