...
with checked as (
    select
        ...
        exists(
            select 1
            from {{ ref('selected_markets') }} as selected_markets
            where selected_markets.market_id = hourly.market_id
        ) as has_selected_market,
        count(*) over (
            partition by hourly.clob_token_id, hourly.odds_hour_utc
        ) as grain_count
    from {{ ref('selected_token_hourly_odds') }} as hourly
)
